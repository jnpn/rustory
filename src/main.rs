...
/***
 *
 * description: sqlite | rust | json
 *
 * 1) sqlite db (id,url,title,...)* -> Thing(i,s)*
 * 2) Thing(i,s)* -> HTTP HEAD(s)* -> ResponsWrapper( )* -> IO JSON
 *
 ***/
...
impl serde::Serialize for ResponseWrapper {
    ...
    where
        S: Serializer,
    {
        let m: HashMap<&str, &str> = self
            .r
            .headers()
            .iter()
            .map(|(k, v)| match (k.as_str(), v.to_str()) {
                (k, Ok(v)) => (k, v),
